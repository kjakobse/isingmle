...
//'
//' \code{calculateNewPAndEHatCheckLikelihood} is intended to be used by \code{IsingMLEmtp2CheckLikelihood}. It updates the distribution p for the edges in ePlus when the empirical distributions don't contain zeroes and prints the likelihood after each iteration.
//'
//' \code{calculateNewPAndEHatCheckLikelihood} doesn't use the value of eHat. It fills the matrix eHat with the edges of the independence graph after the update.\cr
...
//' @return \code{calculateNewPAndEHatCheckLikelihood} returns a list containing the updated distribution, its independence graph, and the canonical parameter J.
//' @export
//'
// [[Rcpp::export]]
List calculateNewPAndEHatCheckLikelihood(NumericMatrix ePlus, int d, NumericMatrix e, NumericVector p, NumericMatrix eHat, NumericMatrix capitalJ, int iter, NumericMatrix data, double logLikelihood) {
...
// The value of the canonical parameter J is calculated for the updated distribution:
  ...
  return List::create(_["p"] = p, _["eHat"] = eHat, _["J"] = capitalJ, _["logLikelihood"] = logLikelihood);
}
