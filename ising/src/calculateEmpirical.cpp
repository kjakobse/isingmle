...
//'
//' \code{CalculateEmpirical} calculates the empirical distribution of pairs of
//' binary variables specified by ePlus from the sample first and second
//' moments.
//'
...
//' The sample moments should be ordered according to this coding, so the
//' sample mean of variable 1 should have index 1 in xBar (using 1-indexing).
//'
...
//' @return \code{CalculateEmpirical} returns a List containing a matrix with
//' the calculated empirical distributions and an integer which is 1 of any of
//' the empirical distributions contain zeroes and is 0 otherwise.
//' @export
//'
// [[Rcpp::export]]
List calculateEmpirical(NumericMatrix ePlus,
                        NumericMatrix m,
                        NumericVector xBar) {
...
// initiate the integer indicating if any empirical distributions contain zeroes:
  ...
  for (int t = 0; t < ePlusDim; t++) {
    ...
    // check if the empirical distribution of pair ij contains zeroes and set
    // containsZeroes to one if it does:
    if (abs(empirical(t, 0)) <= 1e-15 ||
        abs(empirical(t, 1)) <= 1e-15 ||
        abs(empirical(t, 2)) <= 1e-15 ||
        abs(empirical(t, 3)) <= 1e-15) {
      containsZeroes = 1;
    }
  ...
  return List::create(_["empirical"] = empirical,
                      _["containsZeroes"] = containsZeroes);
}
