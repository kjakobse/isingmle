...
//'
//' \code{createEPlus} is intended to be used with \code{isingMLE} and \code{isingMLEmtp2}. It creates the matrix EPlus of edges uv where \eqn{M_uv > \bar{x}_u \bar{x}_v}.
//'
...
NumericMatrix createEPlus(NumericMatrix e, NumericMatrix m, NumericVector xBar) {
  ...
  // Initialise indicator of which rows to include in EPlus and a counter of how many total:
  NumericVector includeRows(nRows);
  int nIncludedRows = 0;
  // Check condition for being in EPlus. If true, set includeRows to 1 and increment total row counter:
