...
//'
//' \code{CalculateEmpiricalReplaceZeroes} calculates the empirical distribution of pairs of binary variables specified by ePlus from the sample first and second moments and adds a small positive constant if a probability is zero.
//'
...
//' The value added to the zeroes is subsequently subtracted from the positive values so they still sum to 1.
//'
...
//'
// [[Rcpp::export]]
NumericMatrix calculateEmpiricalReplaceZeroes(NumericMatrix ePlus, NumericMatrix m, NumericVector xBar, double ReplaceValue) {
  // determine the number of pairs to calculate the empirical distribution of:
  int ePlusDim = ePlus.nrow();
  // initiate the integers counting how many observations in the pair-margin are zero and positive:
  int zerocounter = 0;
  int poscounter = 0;
  ...
  for (int t = 0; t < ePlusDim; t++) {
    ...
    // check which probabilities are zero, if so replace with ReplaceValue and increment the zero-counter, otherwise increment positive-counter:
    if (empirical(t, 0) <= 0) {
    ...
    // check which probabilities are positive, and subtract the number of ReplaceValue's which are added to the zeroes:
    ...
    double empiricalsum = empirical(t, 0) + empirical(t, 1) + empirical(t, 2) + empirical(t, 3);
    if(empiricalsum != 1) {
