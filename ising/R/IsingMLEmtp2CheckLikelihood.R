...
IsingMLEmtp2CheckLikelihood <- function(G, xBar = NULL, M = NULL, data = NULL, epsilon = 1e-4, epsilon2 = 0, maxIter = 100L, zeroReplace = FALSE, ReplaceValue = 1e-10){
  ...
  if (econtainsZeroes | nrow(empirical) == 0 | pcontainszeroes) {
    ...
    while ((iter < maxIter) & (max(abs(mu-xBar)) > epsilon | suppressWarnings(max(condition2) > epsilon) | suppressWarnings(max(condition) > epsilon)) & (newlogLikelihood - logLikelihood > epsilon2)){
      logLikelihood <- newlogLikelihood
      # Update the distribution for each variable pair in ePlus, and update the edges in the fitted graph:
      pAndEHatResult <- calculateNewPAndEHatBoundaryCheckLikelihood(ePlus, d, empirical, p, eHat, data, logLikelihood)
