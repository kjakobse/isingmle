...
#' MTP_2 constraint.
#'
#' \code{IsingMLEmtp2Partial} fits graphical Ising models under MTP_2
#' constraints using an IPS-type algorithm and allows fixing the value of J_ij.
#'
...
#' (using zero-indexing).\cr To fix the value of J_ij choose an initial
#' distribution with the desired value of J_ij and remove the edge ij from G.
#'
#' @param G A list containing a vector with vertices and a matrix containing
#' edges in the rows.
#' @param pstart A vector with the initial distribution that the algorithm
#' should start from.
...
#' the algorithm.
#' @param ptol Numeric value specifying the tolerence for which values in pstart
...
#' zeroes with.
#' @return \code{IsingMLEmtp2Partial} returns a list with the estimated
...
IsingMLEmtp2Partial <- function(G,
  ...
  if (!is.vector(G[[1]]) || !is.matrix(G[[2]])) {
    stop(paste0("G must contain a vector with vertices and a matrix",
  ...
  Xi <- diag(d)
  # initiate p with the provided initial distribution. Duplicate is used to
  # force p to point to a different memory address than pstart,
