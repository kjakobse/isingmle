...
#' @export
likelihoodRatioTest <- function(nSim, p, d, N, largeModel, nullModel, epsilon, GLarge = NULL, GNull = NULL, maxIter = 100L, nCores = 1L, zeroReplace = FALSE, eps = 1e-10) {
  ...
  if (nCores == 1) {
    res <- sapply(nList, bootstrapLikelihoodRatio, p = p, d = d, N = N, largeModel = largeModel, nullModel = nullModel, epsilon = epsilon, GLarge = GLarge, GNull = GNull, maxIter = maxIter, zeroReplace = FALSE, eps = 1e-10)
  } else {
    cl <- makeCluster(nCores)
    res <- parSapply(cl, nList, bootstrapLikelihoodRatio, p = p, d = d, N = N, largeModel = largeModel, nullModel = nullModel, epsilon = epsilon, GLarge = GLarge, GNull = GNull, maxIter = maxIter, zeroReplace = FALSE, eps = 1e-10)
    stopCluster(cl)
  }
