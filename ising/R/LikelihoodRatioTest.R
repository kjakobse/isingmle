...
#'
#' The function uses the parallel implementation in \code{parSapply} to allow running multiple simulations at a time.
...
#' @export
likelihoodRatioTest <- function(nSim, p, d, N, largeModel, nullModel, epsilon, GLarge = NULL, GNull = NULL, maxIter = 100L, nCores = 1L, zeroReplace = FALSE, ReplaceValue = 1e-10) {
  ...
  if (nCores == 1) {
    res <- sapply(nList, bootstrapLikelihoodRatio, p = p, d = d, N = N, largeModel = largeModel, nullModel = nullModel, epsilon = epsilon, GLarge = GLarge, GNull = GNull, maxIter = maxIter, zeroReplace = zeroReplace, ReplaceValue = ReplaceValue)
  } else {
    cl <- makeCluster(nCores)
    res <- parSapply(cl, nList, bootstrapLikelihoodRatio, p = p, d = d, N = N, largeModel = largeModel, nullModel = nullModel, epsilon = epsilon, GLarge = GLarge, GNull = GNull, maxIter = maxIter, zeroReplace = zeroReplace, ReplaceValue = ReplaceValue)
    stopCluster(cl)
  }
